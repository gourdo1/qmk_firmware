...
bool process_record_user(uint16_t keycode, keyrecord_t * record) {
	...
	case TG_ENGCAP:  // Toggle highlighting Non-English letters during CAPSLOCK
		...
		// Key to the left of encoder function (default DEL)
	case LEFTOFENC:
		if (!(user_config.del_right_home_top)) {
			if (!(user_config.ins_on_shft_bkspc_or_del)) {
		...
		// Key below encoder function (default HOME)
	case BELOWENC:
		if (user_config.del_right_home_top) {
			if (!(user_config.ins_on_shft_bkspc_or_del)) {
				static bool inskey_registered;
				...
				} else { // on release of KC_HOME
					// In case KC_INS is still being sent even after the release of KC_HOME
					if (inskey_registered) {
						unregister_code(KC_INS);
						inskey_registered = false;
						return false;
	...
	case WINLOCK:
		if (record -> event.pressed) {
			...
			user_config.double_tap_shift_for_capslock = !keymap_config.no_gui;//Enable double_tap_shift_for_capslock if disabling super key (i.e. in gaming mode, disable double_tap_shift_for_capslock) - capslock behaves like a normal capslock in gaming mode
		} else unregister_code16(keycode);
		break;
